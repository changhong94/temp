...
{
    int ret = 0;
    if ((ret = list_init(&mg->new_res, sizeof(memory_node))) != 0) {
        LOGE(LOG_ERROR, "error initializing memory new_res list");
        goto out;
    }
    if (bypass == 0) {
        if ((ret = list_init(&mg->map_res, sizeof(resource_memory_mg_map_elem))) != 0) {
            LOGE(LOG_ERROR, "error initializing memory map_res list");
            goto out;
        }
    }
    mg->bypass = bypass;
...
{
    memory_node n = {.start_addr = cuda_address, .length = length};
    if (list_append_copy(&mg->new_res, &n) != 0) {
        LOGE(LOG_ERROR, "failed to append to new_res");
        return 1;
    }
    return 0;
...
static void* resource_memory_mg_search_map(resource_mg *mg, void *client_address)
{
    size_t start = 0;
    size_t end;
    size_t mid;
    resource_memory_mg_map_elem *mid_elem;
    long offset=0;
    if (mg == NULL) {
        LOGE(LOG_ERROR, "resource manager mg is NULL");
        return NULL;
    ...
    while (end >= start) {
        ...
        if (mid_elem->client_address > client_address) {
            ...
            if ( mid_elem->client_address + mid_elem->length <= client_address) {
                start = mid+1;
            } else {
    ...
    LOGE(LOG_DEBUG, "memory mg no find: %p", client_address);
    return client_address;
}
...
void resource_mg_print(resource_mg *mg)
{
    ...
    if (mg->bypass == 0) {
        if(mg == &rm_memory){
            resource_memory_mg_map_elem *elem;
            LOG(LOG_DEBUG, "map_res:");
            for (i = 0; i < mg->map_res.length; i++) {
                elem = list_get(&mg->map_res, i);
                LOG(LOG_DEBUG, "%p -> %p, length %ld", elem->client_address, elem->cuda_address, elem->length);
            }
        } else {
...
inline void* resource_mg_get(resource_mg *mg, void* client_address)
{
    if (mg->bypass) {
        ...
        if(mg == &rm_memory) {
            return resource_memory_mg_search_map(mg, client_address);
        } else {
...
int resource_memory_mg_add_sorted(resource_mg *mg, void* client_address, void* cuda_address, size_t mem_size)
{
    ...
    ssize_t end = mg->map_res.length-1;
    ssize_t mid;
    resource_memory_mg_map_elem new_elem = {.client_address = client_address,
                                              .cuda_address = cuda_address,
                                              .length = mem_size};
    resource_memory_mg_map_elem *mid_elem;
    if (mg == NULL) {
    ...
    resource_memory_mg_map_elem *end_elem = list_get(&mg->map_res, end);
    if (end_elem->client_address < client_address) {
        end++;
    }
    return list_insert(&mg->map_res, end, &new_elem);
