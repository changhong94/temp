...
int cricket_restore(int argc, char *argv[])
{
    ...
    execute_command("set exec-wrapper env 'LD_PRELOAD=/home/hwhiaiuser/cchen/experi/cricket/bin/cricket-server.so' 'CRICKET_RESTORE=1'", !batch_flag);
    ...
    uint64_t rb_address;
    bool found_callstack;
    found_callstack = false;
        ...
        for (uint8_t warp = 0; warp != dev_prop.numWarps; warp++) {
            if (warp_mask[sm] & (1LU << warp)) {
                ...
                if (strcmp(warp_kn, kernel_name) != 0) {
                    fprintf(stderr, "Only one kernel can be checkpointed");
                    // change to avoid compilation error, following statement missing arguments anyway.
                    /**
...
int cricket_start(int argc, char *argv[])
{
    ...
    //
    execute_command("set exec-wrapper env 'LD_PRELOAD=/home/hwhiaiuser/cchen/experi/cricket/bin/libtirpc.so.3:/home/hwhiaiuser/cchen/experi/cricket/bin/cricket-server.so'", !batch_flag);
