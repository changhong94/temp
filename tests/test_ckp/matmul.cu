// usage: 
// REMOTE_GPU_ADDRESS=127.0.0.1 LD_PRELOAD=/home/hwhiaiuser/cchen/experi/cricket/bin/cricket-client.so ./test_ckp server-pid
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <sys/time.h>
...
__device__
void inline_matmul(uint16_t *A, uint16_t *x, uint16_t *res)
{
    int i = threadIdx.x;
    int j;
    int r;
...
__global__
void kernel(uint16_t *A, uint16_t *x, uint16_t *res)
{
    inline_matmul(A,x,res);
}
...
int main(int argc, char *argv[])
{
    ...
    dim3 dimGrid( 1, 1);
    kernel<<<dimGrid, dimBlock>>>(dev_A, dev_x, dev_res);
    kernel_no_param<<<dimGrid, dimBlock>>>();
    printf("Kernel Test: Create a checkpoint now!\n");
    if (argc == 2) {
        int pid = atoi(argv[1]);
        if (kill(pid, SIGUSR1) < 0) printf("kill error\n");
    }
    printf("Reinitialize CUDA context\n");
    cudaDeviceReset();

    // verify if cudaDeviceReset already refresh device memory space
    printf("-------------------verifying, expect incorrect result-------------------\n");
    ...
    sleep(5);
    // restore
    printf("-------------------restore, expect correct result--------------------------------------\n");
    if (argc == 2) {
        int pid = atoi(argv[1]);
        if (kill(pid, SIGUSR2) < 0) printf("kill error\n");
    }
    cudaMemcpy(res, dev_res, x_size, cudaMemcpyDeviceToHost );
    cudaFree( dev_A );
    cudaFree( dev_x );
